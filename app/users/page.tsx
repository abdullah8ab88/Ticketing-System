...
export default function UsersPage() {
  ...
  async function deleteUser(uid: string) {
    ...
    try {
      ...
      setDeleteError(t('users.deleteError'));
    } finally {
      setDeleting(false);
    }
  }
      ...
      <AppShell title={t('users.title')} subtitle={t('users.subtitle')}>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              <thead className="text-xs uppercase text-slate-400">
                <tr>
                  <th className="py-3">{t('users.colUser')}</th>
                  <th>{t('users.colDepartment')}</th>
                  <th>{t('users.colDeptVerification')}</th>
                  ...
                  <tr key={u.uid} className="hover:bg-slate-50">
                    ...
                    <td>
                      <select className="input min-w-48" value={u.department} onChange={(e) => updateUser(u.uid, { department: e.target.value, departmentVerificationStatus: 'pending' })}>
                        ...
                        {!u.department && <option value="">{t('users.notSelected')}</option>}
                        {employeeDepartments.map((d) => <option key={d} value={d}>{translateValue(employeeDepartmentLabels, d, lang)}</option>)}
                      </select>
                    </td>
                    <td>
                      <button
                      ...
                      >
                        {(u.departmentVerificationStatus ?? 'verified') === 'pending' ? t('users.verifyDepartment') : t('users.verified')}
                      </button>
                    </td>
                    <td>
                      <select className="input min-w-40" value={u.role} disabled={u.uid === profile?.uid} onChange={(e) => updateUser(u.uid, { role: e.target.value as Role })}>
                        ...
                        <option value="it_manager">{t('users.roleItManager')}</option>
                    ...
                    <td>
                      <button disabled={u.uid === profile?.uid} onClick={() => updateUser(u.uid, { pending: !u.pending })} className={`badge ${u.pending ? 'bg-amber-50 text-amber-700' : 'bg-emerald-50 text-emerald-700'}`}>
                        {u.pending ? t('users.pending') : t('users.active')}
                      </button>
                    </td>
                    <td>
                      <button disabled={u.uid === profile?.uid} onClick={() => { setDeleteError(''); setDeleteTarget(u.uid); }} className="btn-secondary py-2 text-rose-700 disabled:opacity-30">{t('users.delete')}</button>
                    </td>
                  </tr>
                ))}
                {!users.length && <tr><td colSpan={6} className="py-10 text-center text-slate-500">{t('users.noUsers')}</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
        {deleteTarget && <dialog ref={deleteDialog} aria-modal="true" aria-labelledby="delete-title" className="fixed inset-0 z-50 m-0 flex h-full w-full items-center justify-center bg-black/40 p-6" onCancel={(event) => { event.preventDefault(); if (!deleting) setDeleteTarget(null); }}>
          <div className="card max-w-md">
            <h2 id="delete-title" className="text-xl font-bold text-lazem-teal">{t('users.deleteTitle')}</h2>
            <p className="mt-3 text-sm text-slate-600">{t('users.deleteBody')}</p>
            {deleteError && <p role="alert" className="mt-3 text-rose-700">{deleteError}</p>}
            <div className="mt-5 flex gap-3"><button autoFocus className="btn-secondary" disabled={deleting} onClick={() => setDeleteTarget(null)}>{t('users.deleteConfirmCancel')}</button><button className="btn-primary" disabled={deleting} onClick={() => deleteUser(deleteTarget)}>{deleting ? t('users.deleting') : t('users.deleteConfirmSubmit')}</button></div>
          </div>
        </dialog>}
      </AppShell>
    </ProtectedRoute>
