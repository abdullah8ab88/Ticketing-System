...
import { BarChart3, Languages, LogOut, PlusCircle, Ticket, Users } from 'lucide-react';
...
export function AppShell({ children, title, subtitle }: { children: React.ReactNode; title: string; subtitle?: string }) {
  ...
  const nav = [
    { href: '/dashboard', label: t('nav.dashboard'), icon: BarChart3, all: true },
    { href: '/tickets', label: t('nav.tickets'), icon: Ticket, all: true },
    { href: '/tickets/new', label: t('nav.newTicket'), icon: PlusCircle, all: true },
    { href: '/users', label: t('nav.users'), icon: Users, admin: true }
  ];

  return (
    <div className="min-h-screen lg:flex">
      <aside className="brand-gradient fixed inset-y-0 left-0 z-20 hidden w-72 flex-col p-5 text-white lg:flex rtl:left-auto rtl:right-0">
...
<div className="flex h-14 w-20 items-center justify-center rounded-2xl bg-white text-lazem-teal shadow-lg">
  ...
  />
          ...
          <div>
            ...
            <div className="text-xs text-white/70">{t('nav.appSub')}</div>
          </div>
        </div>
        ...
        <div className="mt-auto rounded-3xl bg-white/10 p-4">
          ...
          <div className="mt-3 inline-flex rounded-full bg-white/15 px-3 py-1 text-xs font-semibold">
            {profile?.role ? roleLabels[profile.role]?.[lang] ?? profile.role : ''}
          </div>
      ...
      <main className="w-full lg:pl-72 rtl:lg:pl-0 rtl:lg:pr-72">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              <button
                type="button"
                onClick={toggleLang}
                className="flex items-center gap-2 rounded-xl bg-white px-3 py-2 text-sm font-semibold text-lazem-teal shadow-sm hover:bg-slate-50"
